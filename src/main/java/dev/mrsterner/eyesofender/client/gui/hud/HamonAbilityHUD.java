...
public class HamonAbilityHUD extends DrawableHelper {
	...
	public void render(MinecraftClient client, MatrixStack matrixStack, float tickDelta, int scaledWidth, int scaledHeight, PlayerEntity player) {
		if (AbilityClientHandler.selectedAbility != null) {
			if(EOEComponents.HAMON_COMPONENT.maybeGet(player).isPresent()){
				AbilityUser.of(player).ifPresent(user -> {
					...
					RenderSystem.setShaderTexture(0, SelectAbilityWidget.getTexture(AbilityClientHandler.selectedAbility.hamonLevel));
					...
					RenderSystem.setShaderTexture(0, AbilityClientHandler.selectedAbility.hamonAbility.getTextureLocation());
